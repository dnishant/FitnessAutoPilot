...
export function buildRecipeResolutionPrompt(
  ...
  const userPrompt = [
    ...
    "- mealComponents[]: ONLY edible food identities for sides/condiments the diner may eat separately,",
    "  OR culinary_need entries (kind=culinary_need) describing an abstract accompaniment need.",
    "  NEVER put purpose prose in name (invalid: 'Absorbs pan sauce', 'Complements seafood with crunch').",
    "  Valid edible name examples: 'Roasted Rosemary Potatoes', 'Sautéed Garlic Spinach', 'Basmati Rice'.",
    "  For culinary_need: set kind=culinary_need, put the need text in purpose, use a short role label for name,",
    "  and do NOT invent a fake dish name from the purpose sentence.",
    "  mealComponent.type MUST be one of: main | carb_side | vegetable_side | sauce | condiment | garnish | other",
    "  relationship: intrinsic | recommended_side | optional",

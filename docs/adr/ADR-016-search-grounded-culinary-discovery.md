...
- Implement `GeminiGroundedCulinaryDiscoveryProvider` in `packages/llm` using `@google/genai` with the `googleSearch` tool (not legacy `googleSearchRetrieval`), combined with structured JSON output on Gemini 3.x.
...
- Grounding + structured output relies on Gemini 3 series model support; failures surface as typed `DISCOVERY_NOT_GROUNDED` / provider errors.

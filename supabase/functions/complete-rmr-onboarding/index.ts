...
function mapNutritionTarget(row: Record<string, unknown>) {
  ...
  return {
    ...
    fiberPolicyName: row.fiber_policy_name ?? undefined,
    fiberPolicyVersion: row.fiber_policy_version ?? undefined,
    inputSnapshot: row.input_snapshot,
...
serveWithCors(async (req) => {
  ...
  const { data: nutritionTarget, error: nutritionError } = await service
    ...
    .insert({
      ...
      fiber_g: nutritionFields.fiberG,
      ...
      fiber_policy_name: nutritionFields.fiberPolicyName,
      fiber_policy_version: nutritionFields.fiberPolicyVersion,

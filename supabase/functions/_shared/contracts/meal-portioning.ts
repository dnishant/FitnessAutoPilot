import { z } from "zod";
import { IngredientNutritionSchema, NutrientsPer100gSchema } from "./food-resolution.ts";
import { MealComponentRoleSchema } from "./meal-composition.ts";
import { PersonalizedMealNutritionSchema } from "./consumer-plan.ts";
...
/**
 * PLAN-010 plan-level allocation: reserves breakfast/snack capacity and
 * produces starting meal intents for planned lunch/dinner.
 * Versioned and centralized — do not scatter shares in the solver.
 */
export const NUTRITION_ALLOCATION_POLICY_VERSION = "nutrition-allocation-policy-v1" as const;
export const WEEKLY_PERSONALIZATION_VERSION = "weekly-nutrition-personalization-v1" as const;

export const NutritionBudgetSchema = z.object({
  caloriesKcal: z.number().finite().nonnegative(),
  proteinGrams: z.number().finite().nonnegative(),
  carbsGrams: z.number().finite().nonnegative().optional(),
  fatGrams: z.number().finite().nonnegative().optional(),
  fiberGrams: z.number().finite().nonnegative().optional(),
});

export const NutritionAllocationPolicySchema = z.object({
  version: z.literal(NUTRITION_ALLOCATION_POLICY_VERSION),
  /**
   * Fraction of daily target reserved for breakfast + snacks / unplanned meals.
   * Lunch + dinner intents consume the remainder.
   */
  reservedBreakfastSnackShare: z.number().finite().positive().max(0.6),
  /** Share of the *planned* (non-reserved) budget assigned to lunch. */
  lunchShareOfPlanned: z.number().finite().positive().max(1),
  /** Share of the *planned* (non-reserved) budget assigned to dinner. */
  dinnerShareOfPlanned: z.number().finite().positive().max(1),
  assumptions: z.array(z.string().trim().min(1).max(240)).max(12),
});

export const DailyNutritionBudgetSchema = z.object({
  target: NutritionBudgetSchema,
  reservedNutrition: NutritionBudgetSchema,
  lunchIntent: MealNutritionIntentSchema,
  dinnerIntent: MealNutritionIntentSchema,
  allocationPolicyVersion: z.literal(NUTRITION_ALLOCATION_POLICY_VERSION),
});

export const PersonalizedWeeklyMealInstanceSchema = z.object({
  ...
  completeMealId: z.string().trim().min(1).max(80).optional(),
  mealName: z.string().trim().min(1).max(160),
  /** Starting allocator intent (may be adjusted by daily reconciliation). */
  nutritionIntent: MealNutritionIntentSchema,
  personalizedPlan: PersonalizedMealPlanSchema.optional(),
  status: PortionSolverStatusSchema,
  blockReason: PortionSolverBlockReasonSchema.optional(),
  message: z.string().trim().min(1).max(600).optional(),
});

export const PersonalizedDailyNutritionPlanSchema = z.object({
  day: z.enum([
    "monday",
    "tuesday",
    "wednesday",
  ...
  budget: DailyNutritionBudgetSchema,
  meals: z.array(PersonalizedWeeklyMealInstanceSchema).min(1).max(4),
  plannedNutrition: PersonalizedMealNutritionSchema.optional(),
  reservedNutrition: NutritionBudgetSchema,
  projectedDailyNutrition: PersonalizedMealNutritionSchema.optional(),
  status: PortionSolverStatusSchema,
  message: z.string().trim().min(1).max(600).optional(),
});

export const PersonalizedWeeklyNutritionPlanSchema = z.object({
  generatedPlanId: z.string().trim().min(1).max(120),
  weekStart: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  weekEnd: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  dailyTarget: NutritionBudgetSchema,
  allocationPolicyVersion: z.literal(NUTRITION_ALLOCATION_POLICY_VERSION),
  portionPolicyVersion: z.literal(MEAL_PORTION_POLICY_VERSION),
  personalizationVersion: z.literal(WEEKLY_PERSONALIZATION_VERSION),
  nutritionTargetId: z.string().uuid().optional(),
  nutritionTargetAlgorithmVersion: z.string().trim().min(1).max(80).optional(),
  days: z.array(PersonalizedDailyNutritionPlanSchema).min(1).max(7),
  status: PortionSolverStatusSchema,
  mealInstanceCount: z.number().int().nonnegative(),
  solvedMealCount: z.number().int().nonnegative(),
  bestFeasibleMealCount: z.number().int().nonnegative(),
  blockedMealCount: z.number().int().nonnegative(),
...
export type NutritionBudget = z.infer<typeof NutritionBudgetSchema>;
export type NutritionAllocationPolicy = z.infer<typeof NutritionAllocationPolicySchema>;
export type DailyNutritionBudget = z.infer<typeof DailyNutritionBudgetSchema>;
export type PersonalizedWeeklyMealInstance = z.infer<typeof PersonalizedWeeklyMealInstanceSchema>;
export type PersonalizedDailyNutritionPlan = z.infer<typeof PersonalizedDailyNutritionPlanSchema>;
export type PersonalizedWeeklyNutritionPlan = z.infer<typeof PersonalizedWeeklyNutritionPlanSchema>;
